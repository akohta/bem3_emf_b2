...
void eh_field_x(IMD *id,MOBJ *mo);
void eh_field_y(IMD *id,MOBJ *mo);
void eh_field_z(IMD *id,MOBJ *mo);
void output_field(char *pl,IMD *id,MOBJ *mo);
...
int main(int argc,char *argv[])
{
  ...
  // x=0 plane
  eh_field_x(&id,&mo);
  output_field("yz",&id,&mo);
  // y=0 plane
  eh_field_y(&id,&mo);
  output_field("xz",&id,&mo);
  // z=0 plane
  eh_field_z(&id,&mo);
  output_field("xy",&id,&mo);
...
void eh_field_x(IMD *id,MOBJ *mo)
{
  ...
  for(i=0;i<id->m;i++){
    ...
    for(j=0;j<id->m;j++){
      ...
      mo_EH_t(e,h,x,id->type,mo); // total field
...
void output_field(char *pl,IMD *id,MOBJ *mo)
{
  ...
  dt=mo->md[0].mw.lambda_0/(double)id->ts;
  
  #pragma omp parallel for schedule(dynamic) 
  for(n=0;n<id->ts;n++){
    output_png(n,cexp(-I*mo->md[0].mw.omega*dt*(double)n),pl,id);
  }
